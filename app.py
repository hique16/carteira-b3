...
_cache      = {"html": """<html><head><meta charset='UTF-8'><meta http-equiv='refresh' content='10'><style>body{{font-family:sans-serif;background:#0e0f11;color:#f0f1f3;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:16px}}.spin{{width:40px;height:40px;border:3px solid #333;border-top-color:#7c6af7;border-radius:50%;animation:s .7s linear infinite}}@keyframes s{{to{{transform:rotate(360deg)}}}}</style></head><body><div class='spin'></div><p>Carregando dashboard...</p><p style='font-size:12px;color:#555'>Buscando cotações, aguarde ~1 minuto</p></body></html>""", "lock": threading.Lock()}
...
def connect_sheets():
    ...
    # Corrige quebras de linha escapadas que o Render pode introduzir
    creds_raw = CREDS_JSON.replace("\\n", "\n")
    try:
        creds_dict = json.loads(creds_raw)
    except json.JSONDecodeError:
        # Tenta corrigir private_key com newlines literais
        import re
        creds_raw2 = re.sub(r'("private_key":\s*")(.*?)(")', 
            lambda m: m.group(1) + m.group(2).replace("\n", "\n") + m.group(3),
            creds_raw, flags=re.DOTALL)
        creds_dict = json.loads(creds_raw2)
...
def update_loop():
    # Pequena pausa para o gunicorn terminar de inicializar
    time.sleep(5)
    while True:
        try:
            print(f"[{datetime.now().strftime('%H:%M')}] Atualizando…", flush=True)
            sheet = connect_sheets()
            print(f"[{datetime.now().strftime('%H:%M')}] Sheets: {sheet.title}", flush=True)
            portfolio, divs_received, stops, cal_map = read_from_sheets(sheet)
            print(f"[{datetime.now().strftime('%H:%M')}] {len(portfolio)} ativos lidos", flush=True)
            if portfolio:
                # Busca cotações em lotes menores para evitar timeout
                ...
                with _cache["lock"]:
                    _cache["html"] = html
                print(f"[{datetime.now().strftime('%H:%M')}] ✅ Dashboard atualizado — {len(portfolio)} ativos", flush=True)
            else:
                print("⚠ Nenhum ativo encontrado na aba Central.", flush=True)
        except Exception as e:
            import traceback
...
# ── Inicialização ─────────────────────────────────────────────────────────────
# Inicia o worker em background ao importar o módulo (compatível com gunicorn)
def start_worker():
    if not SHEET_ID or not CREDS_JSON:
        print("❌  SHEET_ID ou GOOGLE_CREDS_JSON não configurados")
        return
    t = threading.Thread(target=update_loop, daemon=True)
    t.start()
    print("Worker de atualização iniciado")

# Executa ao carregar o módulo (funciona com gunicorn)
start_worker()
