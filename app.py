...
def update_loop():
    while True:
        try:
            ...
            print(f"[{datetime.now().strftime('%H:%M')}] Google Sheets conectado: {sheet.title}")
            portfolio, divs_received, stops, cal_map = read_from_sheets(sheet)
            print(f"[{datetime.now().strftime('%H:%M')}] {len(portfolio)} ativos lidos")
            if portfolio:
                ...
                with _cache["lock"]:
                    _cache["html"] = html
                print(f"[{datetime.now().strftime('%H:%M')}] OK — dashboard atualizado com {len(portfolio)} ativos")
            else:
                print("Nenhum ativo encontrado na aba Central.")
        except Exception as e:
            import traceback
